...
## 3단계. 앱 코드에 서버 주소 넣기 (운영자 1회, 1분)

`rentkeeper/index.html` 과 `임대지기/index.html` 에서 아래 줄을 찾아 2단계 주소를 넣고 커밋·푸시합니다.

```js
const EXTRACT_SERVER = '';   // → const EXTRACT_SERVER = 'https://rentkeeper-extract.xxxx.workers.dev';
```

**사용자(임대인)는 아무 설정도 하지 않습니다.** 앱을 열고 계약 추가 → 카메라로 촬영 → ✨ 판독해서 등록 을 누르면
사진이 이 서버를 거쳐 판독되어 임대현황에 자동 등록됩니다.
...
- **판독 서버에 연결할 수 없습니다** → `EXTRACT_SERVER` 주소 오타(https 포함, 끝 `/` 제외) 확인. Worker 화면 → Logs 에서 실시간 오류를 볼 수 있습니다.
- **판독 서버가 아직 설정되지 않았습니다** → 3단계가 아직 안 된 상태입니다.
